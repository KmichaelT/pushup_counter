...
const texxt=document.getElementById('status')
var good = new Audio('Confirm 01.wav');
var Bad = new Audio('Glitch Error.wav');
const pshupanm=document.getElementById('pushup_animation')
const numberofpushups=document.getElementById("counter_txt")
const bdy=document.getElementById('bddy')
const phonne=document.getElementById('phone_view')
...
function drawKeypoints() {
  ...
  if (poses && poses.length > 0) {
    for (let kp of poses[0].keypoints) {
      ...
      if (count == 17) {
        // document.getElementById('body_detection').innerHTML='<h1>full body detected</h1>'
        // bdy.classList.add("green_bg");
        
        phonne.src='camview_live.svg'
        
        // numberofpushups.classList.remove("hidden");
        // numberofpushups.classList.add("visible");
      }
      else {
...
function inDownPosition() {
  ...
  if ((highlightBack == false) && elbowAboveNose && ((abs(elbowAngle) > 70) && (abs(elbowAngle) < 100))) {
    ...
    if (upPosition == true) {
      ...
      // window.speechSynthesis.speak(msg);
      pshupanm.classList.add("hidden");
      good.play()
      numberofpushups.innerHTML=`<h1 class="number">${reps}</h1><h3 class='status_text'>Counting</h3>
      `
...
document.getElementsByClassName('text').innerText = '<h1>OF COURSE alligators rule!</h1>';
